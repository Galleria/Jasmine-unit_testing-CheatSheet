var breakRings = function( data ){
  ...
  while( rings.length > 0 ){
    ...
    depenRings = defineDepenRings( rings );
    ...
    let maxSecondDuplicateArr = [];
    let maxSecondDuplicateValue = depenRings.slice().sort().reverse()[1];
    depenRings.forEach(function(value, pos) {
      if( value === maxSecondDuplicateValue )
        maxSecondDuplicateArr.push( pos );
    });

    if( (maxDuplicateValue===3 && maxSecondDuplicateValue===2) || (maxDuplicateValue===4 && maxSecondDuplicateValue ===3) ){
      if( maxDuplicateArr.length == 1 && (maxSecondDuplicateArr.length < 4 || rings.length > 10) ){
...
console.log( breakRings([[2,5],[3,7],[5,6],[6,7],[9,6],[8,9],[9,7],[1,4],[1,9],[9,5],[2,4],[2,6],[2,3],[9,2],[3,6],[4,5],[1,2]]) )  //5 ****
